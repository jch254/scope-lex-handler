...
const getResponseMessage = (fullGeniusSong: any, audioFeatures?: any) => {
...
const getAttachment = (fullGeniusSong: any, url: string) => ({
...
{ limit : 1 });
  ...
  if (spotifyMedia !== undefined) {
    attachments.push(getAttachment(fullGeniusSong, spotifyMedia.url));
  }

  if (appleMusicMedia !== undefined) {
    attachments.push(getAttachment(fullGeniusSong, appleMusicMedia.url));
  }

  if (soundcloudMedia !== undefined) {
    attachments.push(getAttachment(fullGeniusSong, soundcloudMedia.url));
  }

  if (youtubeMedia !== undefined) {
    attachments.push(getAttachment(fullGeniusSong, youtubeMedia.url));
  }
  ...
  return {
    ...
    dialogAction: {
      ...
      message: {
        contentType: 'PlainText',
        content: getResponseMessage(fullGeniusSong, audioFeatures),
      },
      responseCard: {
